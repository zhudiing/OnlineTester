 ...
 * @LastEditors  : zhudi
 * @LastEditTime : 2023-07-06 11:08:47
 * @FilePath     : /OnlineTester/crtp/crtp.hpp
 */
#pragma once
...
namespace Crtp
{
  ...
  namespace Test
  {
    ...
    void work(Base<Derived> &b) { b.workInterface(); }
    void test()
    {
      DerivedA worker1;
      DerivedB worker2;
    ...
    void what()
    {
      std::cout
          ...
          << "CRTP 是 C++ 中的一种编程技巧，全称为 Curiously Recurring Template "
             "Pattern（奇异递归模板模式），是一种利用模板特化的方式，实现静态多态"
             "的"
             "方法。具体来说，CRTP "
             ...
             "Base`"
             "，然后在该模板类中定义一些函数或成员变量，这些函数或成员变量可以通"
             "过"
             "派生类的类型参数来实现多态。"
          << "\n-------------\n";
    }
